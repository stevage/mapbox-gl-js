...
export class DataDrivenProperty<T> implements Property<T, PossiblyEvaluatedPropertyValue<T>> {
    ...
    evaluate(value: PossiblyEvaluatedValue<T>, globals: EvaluationParameters, feature: Feature): T {
    ...
    evaluate(value: PossiblyEvaluatedValue<?CrossFaded<T>>, globals: EvaluationParameters, feature: Feature): ?CrossFaded<T> {
        if (value.kind === 'source') {
            ...
            return this._calculate(
                value.evaluate({zoom: Math.floor(globals.zoom - 1.0)}, feature),
                value.evaluate({zoom: Math.floor(globals.zoom)}, feature),
                value.evaluate({zoom: Math.floor(globals.zoom + 1.0)}, feature),
                globals);
        } else {
            return value.value;
